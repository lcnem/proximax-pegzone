...
func AddTestnetCmd(
	ctx *server.Context, cdc *codec.Codec, mbm module.BasicManager,
) *cobra.Command {

	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a pxbd testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesi, config, etc.).

Note, strict routability for addresses is turned off in the config file.

Example:
	pxbd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "pxbd",
		"Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "pxbcli",
...
func InitTestnet(
	...
	pxConfig := srvconfig.DefaultConfig()
	pxConfig.MinGasPrices = minGasPrices
	...
	for i := 0; i < numValidators; i++ {
		...
		genAccounts = append(genAccounts, auth.NewBaseAccount(addr, coins, nil, 0, 0))

		//
		accountName := fmt.Sprintf("account%d", i)
		accAddr, accSecret, err := server.GenerateSaveCoinKey(kb, accountName, keyPass, true)
		if err != nil {
			_ = os.RemoveAll(outputDir)
			return err
		}
		fmt.Printf("Created account: %s, %s\n", accAddr.String(), accSecret)

		accInfo := map[string]string{fmt.Sprintf("%s_secret", accountName): accSecret}
		cliPrint, err = json.Marshal(accInfo)
		if err != nil {
			return err
		}
		if err := writeFile(fmt.Sprintf("%v.json", "account_key_seed"), clientDir, cliPrint); err != nil {
			return err
		}
		...
		genAccounts = append(genAccounts, auth.NewBaseAccount(accAddr, accCoins, nil, 0, 0))
		//
		...
		pxConfigFilePath := filepath.Join(nodeDir, "config/pxbd.toml")
		srvconfig.WriteConfigFile(pxConfigFilePath, pxConfig)
	}
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		nodeAppState, err := genutil.GenAppStateFromConfig(cdc, config, initCfg, *genDoc, auth.GenesisAccountIterator{})
		if err != nil {
			return err
		}
...
func writeFile(name string, dir string, contents []byte) error {
	...
	err = tmos.WriteFile(file, contents, 0655)
	if err != nil {
		return err
	}
